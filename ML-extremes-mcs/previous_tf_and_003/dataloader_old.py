...
class DataGenerator(Sequence):
    ...
    def create_binary_mask(self, y, indx, IDindx):
        ...
        """
        tmp_y = xr.open_dataset(f"{self.path_dataID}/mask/mask_{self.msk_var}_ID{IDindx}.nc")[self.msk_var].values
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            for j, VAR in enumerate(self.variable):
                ...
                # Store sample(s)
                X[i,:,:,j] = xr.open_dataset(f"{self.path_dataID}/{VAR}/file_{VAR}_ID{ID}.nc")[self.era5_vars(VAR)].values
